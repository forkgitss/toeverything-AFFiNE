...
const useGenerateUrl = ({ workspaceId, pageId, urlType }: UseSharingUrl) => {
  ...
  const locationState = useLiveData(activeView.location$).state as {
    hash?: string;
  };
  ...
  const url = useMemo(() => {
    ...
    try {
      return new URL(
        `${baseUrl}/${urlType}/${workspaceId}/${pageId}${urlType === 'workspace' && locationState?.hash ? `${locationState?.hash}` : ''}`
      ).toString();
    } catch (e) {
      return null;
    }
  }, [baseUrl, locationState?.hash, pageId, urlType, workspaceId]);
